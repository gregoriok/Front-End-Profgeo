  ...
  return (
    <div className="min-h-screen bg-gray-100 p-10">
      ...
      <div className="flex justify-between items-center mb-8 max-w-6xl mx-auto">
        <div>
          <h2 className="text-3xl font-bold text-gray-800">Minhas Turmas</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {turmas.length > 0 ? (
          turmas.map((turma) => (
            <div key={turma.id_turma || turma.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition p-6 border border-gray-200">
               {/* ... (CONTEÚDO DO CARD IGUAL AO ANTERIOR) ... */}
                <h3 className="text-xl font-bold text-gray-800 mb-2">{turma.nome}</h3>
                <div className="text-sm text-gray-600 space-y-1">
                   <p>Turno: {turma.turno}</p>
                   <p>Alunos: {turma.n_alunos}</p>
                </div>
                <div className="mt-4 pt-4 border-t border-gray-100">
                    <Link to={`/turma/editar/${turma.id_turma || turma.id}`} className="text-blue-600 font-semibold">Gerenciar Turma →</Link>
                </div>
            </div>
          ))
        ) : (
